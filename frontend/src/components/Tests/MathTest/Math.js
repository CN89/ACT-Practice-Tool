...
import Passage from './MathQuestion';
...
function English ({ selectedAnswerPrompt, setSelectedAnswerPrompt }) {
  ...
  useEffect (() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:4000/api/math/question')
  ...
  const updateResponse = async (response) => {
    ...
    try {
      ...
      const userResponse = await axios.get(
        `http://localhost:4000/api/responses/${userId}/${questionId}`
      )
  
      if (userResponse.data) {
        // Update the existing user response document
        const responseData = await axios.put(
          `http://localhost:4000/api/responses/${userResponse.data._id}`,
          { response }
        )
        setData(responseData.data)
      } else {
        // Create a new user response document
        const responseData = await axios.post('http://localhost:4000/api/responses', {
          response,
          userId,
